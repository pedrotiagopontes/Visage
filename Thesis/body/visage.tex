...
\begin{itemize}
\item Falsos Positivos. Zonas da imagem que contém faces podem ser assinaladas como área de interesse, tal como ilustrado em \ref{fig:detecao_erro1};
...
Para uma dada imagem é verificada a existência de uma cara usando o classificador em cascata treinado para o reconhecimento de caras frontais. Caso se verifique a deteção de mais do que uma região de interesse nessa imagem é necessário proceder à re-avaliação de cada uma dessas regiões de forma a determinar em qual se encontra a face que se pretender utilizar. 
...
\item +50 Pontos caso seja encontrada a boca, e no caso de terem sido encontrado olhos, caso a sua posição vertical se encontre a uma distância de pelo menos 20 pixeis dos olhos, de modo a garantir que um olho não é classificado como boca e vice-versa.
...
O desempenho da tarefa de deteção facial numa amostra da biblioteca LFW-a, com e sem remoção de conflitos pode ser vista na tabela \ref{tab:desempenho_detecao}. Nesta amostra estão incluídas as imagens das pessoas com pelos menos 2 imagens, uma vez que apenas essas poderão ser utilizadas na avaliação do desempenho do sistema, utilizando, pelo menos, uma imagem para treino e uma imagem para teste.

\subsection{Alinhamento e Corte da Face}
...
\item Utilizando um classificador em cascata existente na biblioteca \textit{OpenCV} é verificada a posição do olho esquerdo na direito na imagem;
\item São removidos conflitos e falsos positivos nas posições obtidas para o olho esquerdo e direito, determinando as duas posições mais prováveis para cada um dos olhos, das diversas posições obtidas nos passos 1 e 2;
\item É calculado o ponto central das regiões determinadas como mais prováveis para o olho esquerdo e direito, em que $p_esquerdo(x_1,y_1)$ corresponde ao ponto central da região detetada para o olho esquerdo.e  $p_direito(x_2,y_2)$ corresponde ao ponto central da região detetada para o olho direito.
\item É calculado o ângulo de rotação da imagem, a partir da posição determinada para os olhos:
\begin{equation}
\angle rotação = \frac{\arctan(x_1 - x_2, y_1 - y_2) * 180.0}{\Pi}
\end{equation}
\item
\item A imagem é rodada;
\end{enumerate}
...
\begin{figure}[t]
        ...
        \end{subfigure}
        \caption{Imagem recortada e alinhada segundo implementação própria e segundo LFW-a.}\label{fig:alinhamentos}
\end{figure}

\subsubsection*{Remoção Conflitos e Falsos Positivos}
Tal como anteriormente a utilização de um classificador em cascata origina o aparecimento de alguns falsos positivos e conflitos nas regiões obtidas como posição do olho esquerdo, pelo que se revelou necessária a adoção de uma estratégia de remoção dos mesmo para um alinhamento eficaz da face.

Ilustrar

\subsection{Normalização}
Falar das três alternativas (talvez mover para aqui texto do chap avaliacao)
...
Dizer que é importante por remover background.
Mostrar exemplos de paper onde tb é usada.


\section{Módulo \textit{FaceModel}}

Falar dos três algoritmos
Falar da alteracao efetuada para devolver n caras
